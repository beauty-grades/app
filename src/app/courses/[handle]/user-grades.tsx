...
export const UserGrades = ({ course_handle }: UserGradesProps) => {
  ...
  React.useEffect(() => {
          ...
          if (state === "active") {
            if (data?.length > 0) {
              ...
              if (current_period?.grades?.length > 0) {
                ...
                const evaluation_weight_bars = element.querySelectorAll(".evaluation-weight")
                evaluation_weight_bars.forEach((el) => {
                  const [x, label] = el.id.split(":")
                  const ev = resume_grades[label]
                  const final_s = ev.reduce((acc, cur) => acc + cur.score * cur.weight, 0)
    ...
    elements.forEach((element) => element.id.startsWith('period') && observer.observe(element, { attributes: true }))
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        className={"absolute left-0 z-0 h-full bg-gradient-to-r from-red-600 to-purple-600"}
      />
    </AnimatePresence>
  )
}
